    ...
    for (int k = ghost; k < lz - ghost; k++)
    {
        for (int j = ghost; j < ly - ghost; j++)
        {
            for (int i = ghost; i < lx - ghost; i++)
            {
                ...
                usr[i + j * lx + k * lx * ly] = (pow(x_lo + i - ghost - 128, 2) + pow((y_lo + j - ghost - (glx - 1) / 8), 2) + pow((z_lo + k - ghost - (glx - 1) / 8), 2) <= 32) ? 0 : 1;
...
void LBM::exchange()
{
    ...
    if (y_lo != 0)
    {
        for (int k = 0; k < lz; k++)
        {
            MPI_Send(f + (ghost)*q * lx + k * q * lx * ly, lx * q, MPI_DOUBLE, comm.front, k, comm.comm);
        }
    }
    ...
    if (y_hi != gly - 1)
    {
        for (int k = 0; k < lz; k++)
        {
            MPI_Recv(f + (ly - ghost) * q * lx + k * q * lx * ly, lx * q, MPI_DOUBLE, comm.back, k, comm.comm, MPI_STATUSES_IGNORE);
        }
    }
    ...
    if (y_hi != gly - 1)
    {
        for (int k = 0; k < lz; k++)
        {
            MPI_Send(f + (ly - ghost - 1) * q * lx + k * q * lx * ly, lx * q, MPI_DOUBLE, comm.back, k, comm.comm);
        }
    }
    ...
    if (y_lo != 0)
    {
        for (int k = 0; k < lz; k++)
        {
            MPI_Recv(f + (ghost - 1) * q * lx + k * q * lx * ly, lx * q, MPI_DOUBLE, comm.back, k, comm.comm, MPI_STATUSES_IGNORE);
        }
    }
    MPI_Barrier(MPI_COMM_WORLD);
}
