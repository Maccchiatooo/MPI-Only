...
struct LBM
{
    ...
    LBM(MPI_Comm comm_, int sx, int sy, int sz, double &tau, double &rho0, double &u0) : comm(comm_), glx(sx), gly(sy), glz(sz), tau0(tau), rho0(rho0), u0(u0)
    {
        ...
        MPI_Type_vector(1, q * lx * ly, 1, MPI_DOUBLE, &m_face1);
        MPI_Type_commit(&m_face1);

        MPI_Type_vector(lz, q * lx, q * lx * ly, MPI_DOUBLE, &m_face2);
        MPI_Type_commit(&m_face2);

        MPI_Type_vector(ly * lz, q, lx * q, MPI_DOUBLE, &m_face3);
        MPI_Type_commit(&m_face3);
        // line
        MPI_Type_vector(1, q * lx, 1, MPI_DOUBLE, &m_line1);
        MPI_Type_commit(&m_line1);

        MPI_Type_vector(ly, q, q * lx, MPI_DOUBLE, &m_line2);
        MPI_Type_commit(&m_line2);

        MPI_Type_vector(lz, q, lx * ly * q, MPI_DOUBLE, &m_line3);
        MPI_Type_commit(&m_line3);
        // point
