    ...
    if (x_lo == 0)
    {
        for (int k = ghost - 1; k < l_e[2] + 1; k++)
        {
            for (int j = ghost - 1; j < l_e[1] + 1; j++)
            {
                ...
                {
                    if (e[ii * 3] > 0)
                        f[ii + q * (ghost - 1) + q * j * lx + q * k * lx * ly] = f[bb[ii] + q * (ghost + 1) + q * (j + 2 * e[ii * 3 + 1]) * lx + q * (k + 2 * e[ii * 3 + 2]) * lx * ly];
                }
            }
        }
    }
        ...
        for (int k = ghost - 1; k < l_e[2] + 1; k++)
        {
            for (int j = ghost - 1; j < l_e[1] + 1; j++)
            {
                for (int ii = 0; ii < q; ii++)
                {
                    if (e[ii * 3] < 0)
                    {
                        f[ii + q * (l_e[0]) + q * j * lx + q * k * lx * ly] = f[bb[ii] + q * (l_e[0] - 2) + q * (j + 2 * e[ii * 3 + 1]) * lx + q * (k + 2 * e[ii * 3 + 2]) * lx * ly];
                    }
                }
            }
        }
    ...
    for (int k = 0; k < ez; k++)
    {
        for (int j = 0; j < ey; j++)
        {
            for (int i = 0; i < ex; i++)
            {
                ...
                xx[i + j * ex + k * ey * ex] = (double)(x_lo + i) / (glx - 1);
